...
STAGING_AUDIT_FILENAME = "unstep_staging_audit.json"
...
def resolve_attention_backend(
    ...
    *,
    ...
    if requested not in {"auto", "h100", "flash_attn_3", "fa2"}:
        raise ValueError(f"unsupported attention backend: {requested}")
    if requested == "fa2":
        return "fa2"
    if capability is None:
        if not torch.cuda.is_available():
            raise RuntimeError("attention backend selection requires a CUDA device")
        capability = torch.cuda.get_device_capability()
    if requested == "auto":
        return "h100" if capability[0] == 9 else "fa2"
    if capability[0] != 9:
        raise RuntimeError(
            f"{requested} requires a Hopper GPU; use --fa3-import-backend fa2 or auto"
        )
    return requested
...
def patch_attention_fallback(
    ...
    *,
    ...
    backend = resolve_attention_backend(fa3_import_backend) if enable_flash_attn3 else "fa2"
    # Locate upstream import guards independently of its H100/Hopper helper name.
    tree = ast.parse(text)
    guards = []
    for node in tree.body:
        if isinstance(node, ast.Try) and any(
            isinstance(child, ast.Import)
            and any(alias.name in {"flash_attn_interface", "flash_attn_3.flash_attn_interface", "flash_attn"}
                    for alias in child.names)
            for child in ast.walk(node)
        ):
            guards.append(node)
    if len(guards) != 2:
        raise RuntimeError(f"attention import patch did not match {attention_py}; restage native source")
    first, last = guards
    if any(first.lineno < node.lineno < last.lineno and node is not first for node in tree.body):
        raise RuntimeError(f"unexpected statements between attention import guards in {attention_py}")
    new = f"_unstep_attention_backend = {backend!r}\nflash_attn_interface = None\n"
    if backend != "fa2":
        import_line = (
            "import flash_attn_interface" if backend == "h100"
            else "import flash_attn_3.flash_attn_interface as flash_attn_interface"
        )
        new += (
            "if torch.cuda.is_available() and torch.cuda.get_device_capability()[0] == 9:\n"
            f"    {import_line}\n"
        )
    new += """FLASH_ATTN_3_AVAILABLE = flash_attn_interface is not None
try:
    import flash_attn
    FLASH_ATTN_2_AVAILABLE = True
except ModuleNotFoundError:
    FLASH_ATTN_2_AVAILABLE = False
if _unstep_attention_backend == 'fa2' and not FLASH_ATTN_2_AVAILABLE:
    raise RuntimeError('FA2 selected but flash_attn is unavailable; install a compatible FA2 build')
if _unstep_attention_backend != 'fa2' and not FLASH_ATTN_3_AVAILABLE:
    raise RuntimeError('FA3 selected but no compatible Hopper FA3 backend is available')
"""
    lines = text.splitlines(keepends=True)
    text = "".join(lines[:first.lineno - 1]) + new + "".join(lines[last.end_lineno:])
...
def _asset_identity(path: Path) -> dict[str, object]:
    """Detect ordinary asset changes without rereading multi-GB weight contents."""
    if path.is_symlink() and path.is_dir():
        raise RuntimeError(f"directory symlinks are unsupported in staged assets; use a real directory: {path}")
    ...
    for child in sorted(path.rglob("*")):
        if child.is_symlink() and child.is_dir():
            raise RuntimeError(f"directory symlinks are unsupported in staged assets; use a real directory: {child}")
        relative = child.relative_to(path)
        if {".git", "__pycache__"}.intersection(relative.parts) or not child.is_file():
            continue
        stat = child.stat()
        records.append((str(relative), stat.st_size, stat.st_mtime_ns, stat.st_ctime_ns))
    digest = hashlib.sha256(json.dumps(records).encode("utf-8")).hexdigest()
    return {"path": str(path), "file_count": len(records), "file_stat_sha256": digest}


def source_staging_contract(args: Any, manager: Any) -> dict[str, object]:
    assets = {}
    for name in ("source_uri", "weights_uri", "vae_uri", "checkpoint_uri"):
        uri = getattr(args, name, None)
        assets[name] = None if uri is None else _asset_identity(Path(manager.get_local_path(str(uri))))
    settings = {
        name: getattr(args, name, None)
        for name in ("num_frame_per_block", "local_attn_size", "sink_size",
                     "timestep_shift", "skip_cache_cursor_patch")
    }
    settings["attention_backend"] = (
        resolve_attention_backend(getattr(args, "fa3_import_backend", "auto"))
        if getattr(args, "enable_flash_attn3", False) else "fa2"
    )
    return {"assets": assets, "settings": settings}


def read_staging_audit(workdir: Path) -> dict[str, Any]:
    try:
        audit = json.loads((workdir / STAGING_AUDIT_FILENAME).read_text(encoding="utf-8"))
    except (OSError, ValueError) as exc:
        raise RuntimeError("prepared workdir has no valid staging audit; restage without --reuse-workdir") from exc
    if not isinstance(audit, dict) or audit.get("version") != 1:
        raise RuntimeError("unsupported staging audit; restage without --reuse-workdir")
    return audit


def reuse_prepared_source(args: Any, manager: Any, workdir: Path) -> tuple[Path, dict[str, Any]]:
    audit = read_staging_audit(workdir)
    if audit.get("contract") != source_staging_contract(args, manager):
        raise RuntimeError("staged assets or backend/settings changed; restage without --reuse-workdir")
    relative = audit.get("source_root")
    if not isinstance(relative, str):
        raise RuntimeError("staging audit has no source root; restage without --reuse-workdir")
    source_root = (workdir / relative).resolve()
    if not source_root.is_relative_to((workdir / "src").resolve()) or not source_root.is_dir():
        raise RuntimeError("staging audit source root is missing or outside workdir/src")
    if audit.get("staged_source") != _asset_identity(source_root):
        raise RuntimeError("prepared source changed after staging; restage without --reuse-workdir")
    return source_root, audit


def prepare_source(args: Any, manager: Any, workdir: Path) -> Path:
    contract = source_staging_contract(args, manager)
    source_path = Path(manager.get_local_path(str(args.source_uri)))
    source_root = _copy_source_tree(source_path, workdir / "src").resolve()
    ...
    if checkpoint_uri is not None:
        ...
        checkpoint_audit = write_normalized_checkpoint(src, dst)
        (source_root / CHECKPOINT_AUDIT_FILENAME).write_text(
            json.dumps(checkpoint_audit, indent=2, sort_keys=True) + "\n",
            encoding="utf-8",
        )
    ...
    patch_attention_fallback(
        ...
        fa3_import_backend=contract["settings"]["attention_backend"],
    )
    ...
    audit = {
        "version": 1,
        "contract": contract,
        "source_root": str(source_root.resolve().relative_to(workdir.resolve())),
        "staged_source": _asset_identity(source_root),
        "checkpoint_audit": checkpoint_audit,
    }
    (workdir / STAGING_AUDIT_FILENAME).write_text(
...
def runtime_info(
    source_root: Path, precision: str, *, attention_backend: str | None = None,
) -> dict[str, object]:
    ...
    info: dict[str, object] = {
        ...
        "flash_attn_interface_spec": (
            None if attention_backend == "fa2" else has_module("flash_attn_interface")
        ),
        "flash_attn_3_spec": (
            None if attention_backend == "fa2" else has_module("flash_attn_3.flash_attn_interface")
        ),
    }
    ...
    *,
    ...
    def cached_flash_attention(
        ...
        if (
            ...
            if isinstance(x, tuple):
                x = x[0]
            x = x.unflatten(0, (batch_size, q_length))
        else:
