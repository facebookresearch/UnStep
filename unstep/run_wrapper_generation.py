...
_RMTREE_LOG = Path("/tmp/unstep_rmtree_guard.log")
_PROTECTED_RMTREE_ROOTS = (
    ROOT / "results",
    Path("/tmp/unstep_clean_full946"),
)


def guarded_rmtree(path, *args, **kwargs):
    path_resolved = Path(path).resolve(strict=False)
    _RMTREE_LOG.parent.mkdir(parents=True, exist_ok=True)
    with _RMTREE_LOG.open("a", encoding="utf-8") as handle:
        handle.write(f"{time.time():.6f}\trmtree\t{path_resolved}\n")
...
def assert_safe_cleanup_dir(workdir: Path, *, video_dir: Path, out_json: Path) -> None:
...
def assert_safe_speed_video_cleanup_dir(video_dir: Path, *, run_name: str) -> None:
...
def make_prepare_args(args: argparse.Namespace, cfg: WrapperConfig) -> argparse.Namespace:
...
def generation_code_hashes() -> dict[str, dict[str, str]]:
    root = Path(__file__).resolve().parent
    hashes: dict[str, dict[str, str]] = {}
    for name in ("wrapper.py", "wrapper_runtime.py", "run_wrapper_generation.py"):
        path = root / name
        hashes[name] = {
            "path": str(path),
...
def main() -> None:
    ...
    parser.add_argument(
        "--fa3-import-backend",
        default="auto",
        choices=("auto", "h100", "flash_attn_3"),
    )
    ...
    assert_safe_cleanup_dir(workdir, video_dir=video_dir, out_json=out_json)
    if workdir.exists():
    ...
    try:
        ...
        if args.reuse_workdir:
            ...
            if not source_root.exists():
                raise RuntimeError(f"--reuse-workdir requested but src is missing: {source_root}")
        else:
            source_root = runtime.prepare_source(make_prepare_args(args, cfg), manager, workdir)
        ...
        result = {
            ...
            "fa3_import_backend": args.fa3_import_backend,
            "generation_code_hashes": generation_code_hashes(),
            "runtime_info": runtime.runtime_info(source_root, cfg.base.precision),
            "torch_backend_info": {
        ...
        if args.speed_only and args.save_videos:
            assert_safe_speed_video_cleanup_dir(video_dir, run_name=args.run_name)
