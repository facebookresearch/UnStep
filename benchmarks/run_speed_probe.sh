...
if [[ ! "$GPU" =~ ^([0-9]+|GPU-[[:alnum:]-]+|MIG-[[:alnum:]/-]+)$ ]]; then
  echo "CUDA_VISIBLE_DEVICES must expose a GPU for the speed probe." >&2
  exit 2
fi
...
if [[ ! "$WARMUP_REPEATS" =~ ^(0|[1-9][0-9]*)$ ]]; then
  echo "WARMUP_REPEATS must be a non-negative integer." >&2
  exit 2
fi
EXTRA_ARGS=()
CHECKPOINT_URI=""
...
for ((i = 0; i < ${#EXTRA_ARGS[@]}; i++)); do
  case "${EXTRA_ARGS[i]}" in
    ...
    --clean-sigma-override=*|--disable-block-schedule-overrides|--save-videos) ;;
...
RUN_STATE_ROOT="$(unstep_safe_directory cache "$RUN_STATE_ROOT")"
...
WORK_ROOT="$(unstep_safe_directory work "$WORK_ROOT")"
...
if [ -L "$WORK_ROOT.lock" ]; then
  echo "Refusing symlinked staging lock: $WORK_ROOT.lock" >&2
  exit 2
fi
exec 9>>"$WORK_ROOT.lock"
if ! flock -n 9; then
  echo "Speed probe staging is in use: $WORK_ROOT; choose another UNSTEP_WORK_ROOT." >&2
  exit 2
fi
if [ -e "$WORK_ROOT" ]; then
...
print("warm_mean_excluding_first=" + (f"{sum(warm) / len(warm):.4f}" if warm else "n/a"))
print("min_after_first=" + (f"{min(warm):.4f}" if warm else "n/a"))
print("steady_tail_mean=" + (f"{sum(steady_tail) / len(steady_tail):.4f}" if steady_tail else "n/a"))
print("raw_mean=" + (f"{sum(fps) / len(fps):.4f}" if fps else "n/a"))
