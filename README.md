...
If your machine has direct network access, omit `with-proxy`. The launchers use
these paths through `UNSTEP_ASSETS_ROOT`; the source trees can also live
...
```bash
with-proxy huggingface-cli download Wan-AI/Wan2.1-T2V-1.3B --local-dir .build/Wan2.1-T2V-1.3B --local-dir-use-symlinks False
with-proxy huggingface-cli download gdhe17/Self-Forcing checkpoints/self_forcing_dmd.pt --local-dir .build/Self-Forcing --local-dir-use-symlinks False
```
